...
function bindDeepgramEvents(connection: LiveClient | null, socket: Socket, chatbot: any): void {
  ...
  connection.on(LiveTranscriptionEvents.Transcript, async (data: any) => {
    ...
    console.log(`is_final: ${data.is_final} ,  speech_final: ${data.speech_final} ,   ${data.channel.alternatives[0].transcript} , pending tr: ${pendingTranscript}, count: ${emptyTranscriptCount} `);
